...
class Settings:
    ...
    def from_env(cls) -> "Settings":
        public_url = os.getenv("CPS_BRIDGE_URL", "").strip()
        if not public_url:
            raise RuntimeError(
                "Required environment variable CPS_BRIDGE_URL is not set"
            )
        return cls(
            tenant_id=_required("AZURE_TENANT_ID"),
            client_id=_required("CPS_BRIDGE_CLIENT_ID"),
            client_secret=_required("CPS_BRIDGE_CLIENT_SECRET"),
            bridge_audience=_required("CPS_BRIDGE_AUDIENCE"),
