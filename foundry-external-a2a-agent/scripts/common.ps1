...
function Get-FoundryProjectEndpoint {
    ...
    if ($projectId -match "/accounts/([^/]+)/projects/([^/]+)$") {
        return (
            "https://$($Matches[1]).services.ai.azure.com" +
            "/api/projects/$($Matches[2])"
        )
    }
    throw "Unable to resolve the Foundry project endpoint."
}
...
function Reset-AppSecret {
    ...
    $oldIds = @(
        az ad app credential list `
            --id $AppId `
            --query "[?displayName=='$DisplayName'].keyId" `
            --output tsv
    )
    foreach ($oldId in $oldIds) {
        if ($oldId) {
            az ad app credential delete --id $AppId --key-id $oldId
        }
    }
